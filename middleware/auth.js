...
const adminValidateToken = errorfun(async function (req, res, next) {
  ...
  var find_Admin = await UserModel.findOne({
    _id: decode._id,
...
const reCAPTCHA = async (req, res, next) => {
  // console.log(req)
  const token =
    req.body.g_recaptcha_response || req.headers.g_recaptcha_response;
  const secret_key = process.env.SECRATKEY;
  if (!token) {
    return res.status(401).json({
      status: 401,
      message: "Authentication failed: no recaptcha response provided.",
    });
  }
  try {
    axios
      ...
      .then(function (response) {
        if (response.data.success == true) {
          ...
          return res.status(401).json({
            status: 401,
            message: "Authentication failed: timeout-or-duplicate.",
          });
        }
      })
