...
/* GET home page. */
var cpUpload = upload1.fields([{ name: "userImage", maxCount: 1 }]);
...
router.put(
  ...
  errorfun(User.updateUserProfile)
);
// router.get("/getuserprofile",  errorfun(User.getUserProfile))
router.get(
  "/getuserprofile",
  Auth.apiValidateToken,
  errorfun(User.getUserProfile)
);
router.delete(
  "/deleteuser/:id",
  Auth.apiValidateToken,
  errorfun(User.deleteUser)
);
