...
module.exports = {
    addProduct: async (req, res, next) => {
        ...
        var find_prod = await BikeModel.findOne({ productName: req.body.productName });
        if (find_prod) {
            const err = new customError(global.CONFIGS.api.Productalreadyadded, global.CONFIGS.responseCode.alreadyExist);
            next(err);
        }
        var create_prod = await BikeModel.create(req.body);
    ...
    updateProduct: async (req, res, next) => {
        var find_prod = await BikeModel.findOne({ productName: req.body.productName, _id: { $nin: [req.params.id] } });
        if (find_prod) {
            const err = new customError(global.CONFIGS.api.Productalreadyadded, global.CONFIGS.responseCode.alreadyExist);
            next(err);
        }
        var update_prod = await BikeModel.updateOne({ _id: req.params.id }, req.body);
        return res.status(global.CONFIGS.responseCode.success).json({
            success: true,
    ...
    deleteProduct: async (req, res, next) => {
        var delete_prod = await BikeModel.deleteOne({ _id: req.params.id });
        if (delete_prod) {
            return res.status(global.CONFIGS.responseCode.success).json({
                success: true,
    ...
    productListFront: async (req, res, next) => {
        ...
        var productData = await BikeModel.aggregate([
            {
