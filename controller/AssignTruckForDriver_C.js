...
module.exports = {
  asignTruck: async (req, res, next) => {
    ...
    const find_truck = await TruckDetailModel.findOne({
    ...
    const find_truckDriver = await TruckDriverModel.findOne({
  ...
  assignDeleteById: async (req, res, next) => {
    const delete_brand = await AssignTruckForDriverModel.findByIdAndDelete({
  ...
  assignUpdateById: async (req, res, next) => {
    let find_assignTruck = await AssignTruckForDriverModel.findById(
      req.params.id
    );
    ...
    if (req.body.truckId != undefined) {
      let find_truck = await TruckDetailModel.findById(
        new ObjectId(req.body.truckId)
      );
      if (!find_truck) {
        const err = new customError(
          global.CONFIGS.api.truckDetailsInactive,
    ...
    if (req.body.truckDriverId != undefined) {
      let find_truckDriver = await TruckDriverModel.findById(
        new ObjectId(req.body.truckDriverId)
      );
      if (!find_truckDriver) {
        const err = new customError(
          global.CONFIGS.api.truckDriverInactive,
    ...
    find_assignTruck = await AssignTruckForDriverModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
