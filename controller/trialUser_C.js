...
module.exports = {
    addTrailUsers: async (req, res, next) => {
        var find_trialusers = await TrialUserModel.findOne({ mobileNumber: req.body.mobileNumber });
        if (find_trialusers) {
    ...
    trialusersListAdminBYdate: async (req, res, next) => {
        ...
        // var find_trialusersByDate = await TrialUserModel.find({ createdAt: { $gte: date } });
    ...
    //     var find_brand = await TrialUserModel.find({ activeStatus: "1" }).sort({ bikeBrand: 1 });
    //     return res.status(global.CONFIGS.responseCode.success).json({
    //         success: true,
    ...
    //     var delete_brand = await TrialUserModel.deleteOne({ _id: req.params.id });
    //     return res.status(global.CONFIGS.responseCode.success).json({
    //         success: true,
