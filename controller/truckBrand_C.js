...
module.exports = {
  addBrand: async (req, res, next) => {
    const find_brand = await TruckBrandModel.findOne({
  ...
  updateBrand: async (req, res, next) => {
    ...
    const existing_brand = await TruckBrandModel.findOne({
      truckBrand: req.body.truckBrand,
    ...
    find_brand = await TruckBrandModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
  ...
  brandDelete: async (req, res, next) => {
    const delete_brand = await TruckBrandModel.findByIdAndDelete({
      _id: req.params.id,
    });
    if (!delete_brand) {
      const err = new customError(
  ...
  brandListFront: async (req, res, next) => {
    const find_brand = await TruckBrandModel.find({ activeStatus: "1" }).sort({
      truckBrand: 1,
    });
    const total = find_brand.length;
    return res.status(global.CONFIGS.responseCode.success).json({
