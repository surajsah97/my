...
module.exports = {
    ...
    updateModel: async (req, res, next) => {
        ...
        const existing_model = await BikeModelModel.findOne({ bikeModel: req.body.bikeModel, _id: { $nin: [req.params.id] } });
        if (existing_model) {
            const err = new customError(global.CONFIGS.api.modelalreadyadded, global.CONFIGS.responseCode.alreadyExist);
            return next(err);
        };
        if(req.body.bikeBrandId!=undefined){
            let find_brand = await BikeBrandModel.findById(new ObjectId(req.body.bikeBrandId));
            if (!find_brand) {
            const err = new customError(global.CONFIGS.api.brandInactive, global.CONFIGS.responseCode.notFound);
            return next(err);
            }
        ...
        find_model = await BikeModelModel.findByIdAndUpdate( req.params.id , req.body,{new:true});
        return res.status(global.CONFIGS.responseCode.success).json({
            success: true,
    ...
    modelDelete: async (req, res, next) => {
        var delete_model = await BikeModelModel.findByIdAndDelete({ _id: req.params.id });
        if(!delete_model){
