...
module.exports = {
  addmodel: async (req, res, next) => {
    var find_brand = await TruckBrandModel.findOne({
      _id: req.body.truckBrandId,
      activeStatus: "1",
    });
    if (!find_brand) {
    ...
    var find_model = await TruckModelModel.findOne({
  ...
  updateModel: async (req, res, next) => {
    ...
    const existing_model = await TruckModelModel.findOne({
      truckModel: req.body.truckModel,
    ...
    if (req.body.truckBrandId != undefined) {
      let find_brand = await TruckBrandModel.findById(
        new ObjectId(req.body.truckBrandId)
      );
      if (!find_brand) {
        const err = new customError(
          global.CONFIGS.api.brandInactive,
    ...
    find_model = await TruckModelModel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
  ...
  modelDelete: async (req, res, next) => {
    var delete_model = await TruckModelModel.findByIdAndDelete({
      _id: req.params.id,
    });
    if (!delete_model) {
      const err = new customError(
